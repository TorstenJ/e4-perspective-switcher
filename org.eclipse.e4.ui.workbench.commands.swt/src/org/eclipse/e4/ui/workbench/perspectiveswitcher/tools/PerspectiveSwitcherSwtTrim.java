...
public class PerspectiveSwitcherSwtTrim implements IPerspectiveSwitcherControl {
  ...
  @Inject
  @Preference(value = E4PerspectiveSwitcherPreferences.SHOW_NEW_BUTTON, nodePath = E4PerspectiveSwitcherPreferences.ROOT_PREFERENCES_NODE)
  private boolean showNewButton;

  @Inject
  @Preference(value = E4PerspectiveSwitcherPreferences.ENABLE_MENU, nodePath = E4PerspectiveSwitcherPreferences.ROOT_PREFERENCES_NODE)
  ...
  Color getContainerCurveColor(PaintEvent event) {
    ...
    if (curveColor == null) {
      curveColor = internalCrvColor != null ? internalCrvColor : new Color(event.display, defaultContainerCurveColor);
    }
  ...
  private void addSaveAsMenuItem(Menu menu) {
    ...
    menuItem.addSelectionListener(new SelectionAdapter() {
      ...
      @Override
      public void widgetSelected(SelectionEvent event) {
        ParameterizedCommand command = commandService.createCommand(E4WorkbenchCommandConstants.PERSPECTIVES_SAVE_AS,
  ...
  private void addResetMenuItem(Menu menu) {
    ...
    menuItem.addSelectionListener(new SelectionAdapter() {
      ...
      @Override
      public void widgetSelected(SelectionEvent event) {
        ParameterizedCommand command = commandService.createCommand(E4WorkbenchCommandConstants.PERSPECTIVES_RESET,
            Collections.EMPTY_MAP);
        handlerService.executeHandler(command);
      }
    });
  ...
  private void addCloseMenuItem(Menu menu) {
    ...
    menuItem.addSelectionListener(new SelectionAdapter() {
      ...
      @Override
      public void widgetSelected(SelectionEvent event) {
        ParameterizedCommand command = commandService.createCommand(E4WorkbenchCommandConstants.PERSPECTIVES_CLOSE,
            Collections.EMPTY_MAP);
        handlerService.executeHandler(command);
      }
    });
  ...
  private void addShowTextMenuItem(Menu menu) {
    ...
    menuItem.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent event) {
        ...
        ParameterizedCommand command = commandService.createCommand(E4WorkbenchCommandConstants.PERSPECTIVES_SHOW_TEXT,
            parameters);
        handlerService.executeHandler(command);
      }
    });
