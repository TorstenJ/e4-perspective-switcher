...
/**
 * Takes care of handling additions & removals of perspectives, name changes and the like.
 *
 */
@Creatable
public class EPerspectiveSwitcher {
  ...
  private EventHandler toBeRenderedHandler = new EventHandler() {
    ...
    public void handleEvent(Event event) {
      ...
      // If a perspective does not have a context, it shall be removed
      IEclipseContext cxt = perspective.getContext();
      if (cxt == null) {
        control.removePerspectiveShortcut(perspective);
        return;
      }
      MWindow perspectiveWindow = cxt.get(MWindow.class);
      if (window != perspectiveWindow) {
        return;
      }
  ...
  /**
   * Sets the UI control associated with the perspective switcher. The perspective switcher subscribes to topics only
   * once a non-null ui control is passed and unsubscribes from all topics if a null value is received.
   *
