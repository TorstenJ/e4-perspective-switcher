 ...
 */
@Creatable
public class PerspectiveLabelProvider extends LabelProvider implements
		ITableLabelProvider {
		...
		if (element instanceof MPerspective) {
			...
			try {
				...
				descriptor = (ImageDescriptor) resourceUtilities
						.imageDescriptorFromURI(iconURI);
		...
		if (element instanceof MPerspective) {
			...
			if (markActive && E4Util.isSelectedElement(perspective)) {
				label = label
						...
						+ translationService
								.translate(
										"active", "platform:/plugin/" + E4PerspectiveSwitcherActivator.PLUGIN_ID) + ")"; //$NON-NLS-1$
			}
		}
		return label;
	}
